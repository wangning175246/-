...
**spring-boot-starter**-web：是web的场景启动器，
...
默认使用application.properties的配置，也可以使用在application.properties配置激活那个文件

​	spring.properties.active=dev/或者prod或者其他profile
...
springboot 启动会扫描以下位置的application.properties或者application.yml文件作为Spring boot的默认配置文件，默认是resources目录下，

–file:./config/ 表示resource中的config目录
...
我们还可以通过配置文件中spring.config.location来改变默认的配置文件位置 
...
jar包的外的配置就是 可以通过后  --spring.config.location 指定，

如果没有指定，默认会加载当前jar包同一级目录下的application.properties或application.yaml文件配置
...
**由jar包外向jar包内进行寻找； jar包外的需要和jar在同一级目录。**
...
logging.level.com.atguigu=trace
	#com.atguigu 设置com.atguigu这个包下的日志级别是trace
# logging.level.root=trace 是设置程序的所有包的日志级别，默认基本是info/
#logging.path=
...
`Spring.resources.static-locations=classpath:/,classpath:/resource`

如果配置文件中指定了则，就不用使用默认的配置
